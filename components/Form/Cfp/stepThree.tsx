...
function StepThree({ setStep, setForm, data }: { setStep: any, setForm: any, data: any }) {
  ...
  return (
    <form className="mt-3 w-[30rem] lg:w-[auto]" onSubmit={(e) => setStep(e, 4)}>
      ...
      <div className="mt-3 border w-full border-solid border-y-dark-600 divide-y" />
        ...
        <Select
          options={options1}
          title={{value: "Select session format", label: "Select session format"}}
          setValue={(val: any) => setValue({...value, Format: val})}
          multi={false}
        ...
        <Select
          options={options2}
          title={{value: "Select session level", label: "Select session level"}}
          setValue={(val: any) => setValue({...value, Level: val})}
          multi={false}
