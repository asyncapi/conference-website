...
type StepFourProps = {
  setStep: (e: any | null, step: any) => void;
  setForm: (formData: Record<string, any>) => void;
  data: Record<string, any>;
};

function StepFour({ setStep, setForm, data }: StepFourProps) {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
    try {
      const res = await axios.post(`/api/speakers/register`, data);
      if (res.status === 200) {
        setDisabled(true);
        setStep(e, "successful");
      }
    } catch (err) {
      toast.error("Failed to submit feedback. Try again", { duration: 5600 });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <form className="mt-3 w-[30rem] lg:w-[auto]" onSubmit={onSubmit}>
