...
function Paper() {
  ...
  const onStepUpdate = (e: React.FormEvent<HTMLFormElement>, step: number) => {
    if (e) {
      e.preventDefault();
    }
    setStep(step);
  ...
  useEffect(() => {
    setHeight(confetiRef.current?.clientHeight || 0);
    setWidth(confetiRef.current?.clientWidth || 0);
                ...
                >
                  <div className="flex justify-between">
                    <div className="sm:hidden my-4">
                      <h3 className={`font-bold text-lg ${(index <= (step as number) && "text-[#E50E99]") || ("text-white")}`}>
                        {field.title}
                      </h3>
