...
function Navbar() {
  ...
  const handleClosing = useCallback(
    (event: MouseEvent) => {
      if (show && !(event.target && (event.target as HTMLElement).closest(".subMenu"))) {
        setShow(null);
      }
    },
    [show],
        ...
        <div className="p-5 flex justify-between h-[75px] w-full items-center">
          ...
          {isTablet ? (
            ...
            <div className="flex items-center">
              {links.map((link: { title: string, ref: string, subMenu?: { title: string, ref: string, target?: string }[] }) => (
                <Link href={link.ref} key={link.title}>
                  ...
                  >
                    ...
                    {show === link.title && link.subMenu && (
                      ...
                      >
                        {link.subMenu.map((subL: { title: string, ref: string, target?: string }) => (
                          <Link
                            href={subL.ref}
                            target={subL.target}
                            key={subL.title}
