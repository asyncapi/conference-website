...
module.exports = async (link, title) => {
  ...
  try {
    newCampaign = await mailchimp.campaigns.create({
      ...
      settings: {
        ...
        preview_text:
          "Check out the latest topic that TSC members have to be aware of",
  ...
  try {
    await mailchimp.campaigns.setContent(newCampaign.id, {
      html: htmlContent(link, title),
    });
  } catch (error) {
    return core.setFailed(
      `Failed adding content to campaign: ${JSON.stringify(error)}`,
    );
  }
  ...
  try {
    ...
    const scheduleDate = new Date(
      Date.parse(new Date(Date.now()).toISOString()) + 1 * 1 * 60 * 60 * 1000,
    );
