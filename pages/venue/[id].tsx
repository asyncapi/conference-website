...
export async function getStaticProps({ params }: { params: { id: string } }) {
  let res: any = {} ;
  const data = cities.filter((p: any) => p.name === params.id);
  res = data[0];
  const getSpeakers = speakers.filter((s: Speaker) => s.city === res?.name);
...
function Venue({ city }: VenueProps) {
  ...
  return (
    <div data-test={`venue-${city.name}`}>
      <div className={`w-full h-[500px] sm:h-[auto] ${city.name === 'Online' ? 'bg-online' : 'bg-madrid'} bg-cover bg-center`}>
        <div className='w-full h-full kinda-dark items-center flex flex-col justify-between'>
          <div className='mt-[68px] container text-center flex flex-col items-center w-[1100px] lg:w-full sm:text-center'>
            ...
            <Heading typeStyle='heading-sm' className={`${textColor} mt-[24px] underline`}>
              <a href={city.map} target='_blank' rel='noreferrer'>
                {city.address}
              </a>
            </Heading>
            <Heading typeStyle='heading-sm' className={`${textColor} mt-[24px]`}>
              {city.date}
            </Heading>
            {city.ended ? (
              city.playlist && (
                <a href='#recordings'>
                  <Button className='w-[250px] h-[50px] m-8'>Watch Recordings</Button>
                </a>
              )
            ) : (
              <div className='m-[30px]'>
                ...
                {!eventEnded && city.cfp && (
                  <a href={city.name === 'online' ? '/venue/online/register' : (city.cfp as string)} target={city.name === 'Online' ? '' : '_blank'} rel='noreferrer'>
      ...
      <div id='agenda' className='border border-x-0 border-b-0 border-t-[#333] py-28 container flex flex-col justify-center items-center '>
        {city.cfp ? (
          <div className='w-[1090px] lg:w-full'>
            <Guidelines talkDeadLine={city.name === 'Online' && CFPdata.CallEndDate ? CFPdata.CallEndDate : city.cfpdate} virtual={city.name === 'Online'} name={city.name} cfp={city.cfp} />
          </div>
        ) : (
