...
export default function Home() {
  ...
  const [city, setCity] = useState<City>({
    city: speakers[0].city,
    location: speakers[0].location,
    lists: [],
    cfp: false
  });
    ...
    if (speakers[0]) {
      ...
      speakers.forEach((speaker) => {
        if (Array.isArray(speaker.lists) && Object.keys(speaker.lists).length > 0) {
          initialCity.lists.push(...speaker.lists);
        }
      });

      // Remove duplicates
      const list = initialCity.lists.filter((obj, index) => {
        return index === initialCity.lists.findIndex((o) => obj.name === o.name);
      });
      initialCity.lists = [...list];

      setCity(initialCity);
      setSpeakersList(initialCity.lists);
    }
  }, []);
