...
import { SpeakerCityType, SpeakerListType } from "../types/type";

export default function Home() {
  const isTablet = useMediaQuery({ maxWidth: "1118px" });
  const [speakersList, setSpeakersList] = useState<SpeakerListType[]>([]);
  const [city, setCity] = useState<SpeakerCityType | null>(null);

  // Ensuring speakers[0] is properly initialized and lists are unique
  useEffect(() => {
    const firstSpeaker = speakers[0] as SpeakerCityType;  // Ensure first speaker is defined with the correct type
    if (firstSpeaker) {
      firstSpeaker.lists = [];
      speakers.forEach((speaker: SpeakerCityType) => {
        if (Array.isArray(speaker.lists)) {
          firstSpeaker.lists.push(...speaker.lists);
        }
      });
      const uniqueLists = firstSpeaker.lists.filter((obj, index) => {
        return index === firstSpeaker.lists.findIndex((o) => obj.name === o.name);
      });
      firstSpeaker.lists = [...uniqueLists];

      setCity(firstSpeaker);
      setSpeakersList(firstSpeaker.lists);
    }
  }, []);
    ...
    <div>
      ...
      <div id="register" className="container mt-20">
        <div className="flex items-center flex-col justify-center">
          <div id="speakers" className="relative flex flex-col items-center justify-center">
                ...
                {isTablet ? (
                  ...
                  <div className="flex justify-center">
                    <div className="w-[900px] lg:w-full flex justify-between">
                      {speakers.map((speaker: SpeakerCityType) => {
                        return (
                          <div
                          ...
                          >
                            <Button
                              className={`w-[120px] ${
                                city?.city === speaker.city ? "gradient-bg" : "border border-gray btn relative overflow-hidden transition-all rounded group py-1.5 px-2.5"
                              }`}
                              overlay={true}
                            >
                ...
                {speakersList.length > 0 ? (
                  <div className="w-full grid grid-cols-3 lg:grid-cols-2 sm:grid-cols-1 gap-4">
                    {speakersList.map((speaker, i) => (
                      <Speaker key={i} details={speaker} location={city} className="mt-10" />
                    ))}
                  </div>
                ) : (
